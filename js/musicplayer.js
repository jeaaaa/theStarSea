...
var MPlayer = (function () {
	function MPlayer(settings,callback) {
		...
		$.each($this.list, function (i) {
			listHTML += '<li class="wp-list-title-'+ i +'">' + $this.list[i].listName + '</li>';
		});
		...
		function init (callback) {
			...
			$this.dom = {
				container:con,
				cover:con.find('.wp-cover'),
				name:con.find('.wp-name'),
				singer:con.find('.wp-singer'),
				currentTime:con.find('.wp-time-current'),
				allTime:con.find('.wp-time-all'),
				prev:con.find('.wp-prev'),
				play:con.find('.wp-pause'),
				next:con.find('.wp-next'),
				vol:con.find('.wp-vol-img'),
				volRange:con.find('.wp-vol-range'),
				progress:con.find('.wp-pro-current'),
				progressAll:con.find('.wp-pro'),
				lrc:con.find('.wp-lrc'),
				listTitle:con.find('.wp-list-title'),
				list:con.find('.wp-list')
			};
...
//			});
			...
			$this.audio.on('canplay',function () {
				...
				if (dataLrc != currentLrc) {
					...
					// 删除以前添加的class
					$this.dom.lrc.find('.wp-lrc-current').removeClass('wp-lrc-current');
					// 添加class
					var position = $this.dom.lrc.find('.wp-lrc-time-'+currentLrc).addClass('wp-lrc-current').position();
					// 计算滚动的高度
					var positionTop = position ? position.top+30 : 0;
			...
			// 禁止点击
			$this.dom.container.on('mousedown','.wp-disabled', function () {
				return false;
			})
		}
	...
	$.extend(MPlayer.prototype,{
		...
		_setLrc: function (lrc) {
			...
			$.each(lrc, function (index,val) {
				$this.dom.lrc.append($('<li></li>').addClass('wp-lrc-time-'+index).html(val));
			});
		},
		/**
		 * 更换播放器的歌曲信息
		...
		_setCurrent: function (song) {
			...
			items.find('.wp-list-current').removeClass('wp-list-current');
			items.children().eq(song).addClass('wp-list-current');
		},
		/**
		 * 解析歌词字符串
		 * @param lrc
		...
		play: function (list,song) {
			...
			if (list === undefined && song === undefined) {
				...
				if (flag === false) {
					...
					$this.dom.play.addClass('wp-play');
				}
		...
		pause: function () {
			var $this = this;
			$this.dom.play.removeClass('wp-play');
		...
		changeList: function (list) {
			var $this = this;
			// 设置列表活动类
			$this.dom.listTitle.find('.wp-list-title-current').removeClass('wp-list-title-current');
			$this.dom.listTitle.find('.wp-list-title-'+list).addClass('wp-list-title-current');
			// 切换audio标签的显示列表
		...
		cancelMute: function () {
			...
			$this.dom.vol.removeClass('wp-mute');
			$this._trigger('mute');
		},
		/**
		 * 切换静音状态
...
function setEffects (player) {
	player.dom.container.find('.wp-mode').click(function () {
		var dom = $(this);
		var mode = player.getPlayMode();
		dom.removeClass('wp-mode-'+mode);
		mode = mode == 3 ? 0 : mode + 1;
		player.changePlayMode(mode);
		dom.addClass('wp-mode-' + mode);
	});
	player.dom.container.find('.wp-list-toggle').click(function () {
		player.dom.container.find('.wp-list-box').toggleClass('wp-list-show');
	});
	player.dom.container.find('.wp-lrc-toggle').click(function () {
		player.dom.container.find('.wp-lrc-box').toggleClass('wp-lrc-show');
	});
	player.dom.container.find('.wp-toggle').click(function () {
		player.dom.container.toggleClass('wp-show');
	});
	player.dom.container.find('.wp-lrc-close').click(function () {
		player.dom.container.find('.wp-lrc-box').removeClass('wp-lrc-show');
	});
}
}
